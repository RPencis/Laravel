        ...
        <table class="table mt-4">
            ...
            <tbody>
                ...
                @foreach ($comments as $index => $comment)
                    <tr
                        class="{{ $updated != $comment->id ? '' : ($comment->approved ? 'table-success' : 'table-danger') }}">
                        ...
                        <td>
                            @if(auth()->user()->id === $comment->user_id)
                                <a href="#" disabled class="btn btn-sm btn-outline-success disabled">{{ $comment->approved ? 'Unapprove' : 'Approve' }}</a>
